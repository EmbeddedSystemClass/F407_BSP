 ...
 * @LastEditTime: 2019-05-01 15:42:23
 */
/*
...
#if !defined(BSP_USING_UART1) && !defined(BSP_USING_UART2) && !defined(BSP_USING_UART3) && !defined(BSP_USING_UART4) && !defined(BSP_USING_UART5) && !defined(BSP_USING_UART6) && !defined(BSP_USING_LPUART1)
...
#ifdef RT_SERIAL_USING_DMA
    case RT_DEVICE_DMA_CONFIG:
        //has rx dma
        if (uart->uart_dma_flag == 1)
        {
...
#if defined(SOC_SERIES_STM32L4) || defined(SOC_SERIES_STM32F7) || defined(SOC_SERIES_STM32F0) || defined(SOC_SERIES_STM32L0) || defined(SOC_SERIES_STM32G0) || defined(SOC_SERIES_STM32H7)
...
static rt_size_t stm32_transmit_dma(struct rt_serial_device *serial, rt_uint8_t *buf, rt_size_t size, int direction)
{
    if (RT_SERIAL_DMA_TX == direction)
    {
        HAL_UART_Transmit_DMA(&((struct stm32_uart *)serial->parent.user_data)->handle, buf, size);
    }
    return size;
}
static const struct rt_uart_ops stm32_uart_ops =
    {
        ...
        .putc = stm32_putc,
        .getc = stm32_getc,
        .dma_transmit = stm32_transmit_dma,
};
...
static void uart_isr(struct rt_serial_device *serial)
{
    ...
    uint32_t isrflags = READ_REG(uart->handle.Instance->SR);
    uint32_t cr1its = READ_REG(uart->handle.Instance->CR1);
    uint32_t cr3its = READ_REG(uart->handle.Instance->CR3);
...
#ifdef RT_SERIAL_USING_DMA
    else if ((uart->uart_dma_flag) && ((isrflags & UART_FLAG_IDLE) != RESET) && ((cr1its & UART_IT_IDLE) != RESET))
    {
        level = rt_hw_interrupt_disable();
        recv_total_index = serial->config.bufsz - __HAL_DMA_GET_COUNTER((uart->handle.hdmarx));
...
void UART1_DMA_RX_IRQHandler(void)
{
    ...
    HAL_DMA_IRQHandler(uart_obj[UART1_INDEX].handle.hdmarx);
...
void UART1_DMA_TX_IRQHandler(void)
{
    ...
    HAL_DMA_IRQHandler(uart_obj[UART1_INDEX].handle.hdmatx);
...
void UART2_DMA_RX_IRQHandler(void)
{
    ...
    HAL_DMA_IRQHandler(uart_obj[UART2_INDEX].handle.hdmarx);
...
void UART2_DMA_TX_IRQHandler(void)
{
    ...
    HAL_DMA_IRQHandler(uart_obj[UART2_INDEX].handle.hdmatx);
...
void UART3_DMA_RX_IRQHandler(void)
{
    ...
    HAL_DMA_IRQHandler(uart_obj[UART3_INDEX].handle.hdmarx);
...
void UART4_DMA_RX_IRQHandler(void)
{
    ...
    HAL_DMA_IRQHandler(uart_obj[UART4_INDEX].handle.hdmarx);
...
void UART5_DMA_RX_IRQHandler(void)
{
    ...
    HAL_DMA_IRQHandler(uart_obj[UART5_INDEX].handle.hdmarx);
...
void UART6_DMA_RX_IRQHandler(void)
{
    ...
    HAL_DMA_IRQHandler(uart_obj[UART6_INDEX].handle.hdmarx);
...
void LPUART1_DMA_RX_IRQHandler(void)
{
    ...
    HAL_DMA_IRQHandler(uart_obj[LPUART1_INDEX].handle.hdmarx);
...
static void stm32_dma_config(struct rt_serial_device *serial)
{
    ...
    LOG_D("%s dma RX instance: %x", uart->config->name, uart->handle.hdmarx);
    ...
    for (int i = 0; i < obj_num; i++)
    {
        ...
        if (uart_obj[i].uart_dma_flag)
        {
            ...
            result = rt_hw_serial_register(&uart_obj[i].serial, uart_obj[i].config->name,
                                           RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX | RT_DEVICE_FLAG_DMA_RX | RT_DEVICE_FLAG_DMA_TX, &uart_obj[i]);
        }
        else
#endif
        {
